...
class Employees(models.Model):
    ...
    def is_anonymous(self):
        return not (self.user.is_staff if self.user else False)
...
class TemporaryToken(models.Model):
    ...
    @classmethod
    def generate_signup_token(cls, email):
...
class Patient(models.Model):
    ...
    def add_observation(self, SOAP):
        ...
        if not self.folder:
            raise ValueError("Patient folder is not set.")
    ...
    def list_patient_folder(self):
        folder_path = os.path.join('patients', str(self.patient_id))
...
class Order(models.Model):
    ...
    def check_availability(self):
        ...
        return f"All medications for {self.prescription} are available"
